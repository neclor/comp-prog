...
static void test_course_append(void) {
   ...
   Escale *charleroi = escale_create("Charleroi", 4.4446, 50.4108);

   Course* course = course_create(liege, namur);

   assert_ulong_equal(2, course_get_escales_count(course));
   ...
   assert_ulong_equal(4, course_get_escales_count(course));

   course_append(course, charleroi);

   assert_ulong_equal(5, course_get_escales_count(course));
